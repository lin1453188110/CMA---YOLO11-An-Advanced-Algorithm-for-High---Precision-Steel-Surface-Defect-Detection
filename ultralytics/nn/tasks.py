...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m,(Detect,Detect_ASFF,Detect_FASFF)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_ASFF)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            Segment, Pose,  OBB)) else self.forward(x)
            try:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C3k2 and scale in "mlx":  # for M/L/X sizes
                args[3] = True
        elif m in {MultiDilatelocalAttention}:#注意力
            c2 = ch[f]
            args = [c2, *args]
        elif m is AIFI:
            args = [ch[f], *args]
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect,Detect_ASFF,Detect_FASFF}:
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect,Detect_ASFF)):
                return "detect"
